...
class LogChamp(Cog):
    ...
    async def on_raw_message_edit(self, payload):
        ...
        content = (
            f"{timestamp()} 📝 message by {format_user(author)} in <#{cid}> has been edited (sent about {ago(created_at)} ago)\n"
            f"**Old**: {before}\n"
            f"**New**: {after}"
        )
    ...
    async def on_raw_message_delete(self, payload):
        ...
        created_at = discord.utils.snowflake_time(payload.message_id)
        content = (
            f"{timestamp()} 🗑 message by {format_user(author)} in <#{cid}> has been deleted (sent about {ago(created_at)} ago)\n"
            f"**Content**: {before}"
        )
